...
-- -- datauser role is created by Docker, so we just configure it here
-- DO $$
-- BEGIN
--   IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'datauser') THEN
...
-- Create gatherapi role if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'gatherapi') THEN
    CREATE ROLE gatherapi;
  END IF;
END
$$;
